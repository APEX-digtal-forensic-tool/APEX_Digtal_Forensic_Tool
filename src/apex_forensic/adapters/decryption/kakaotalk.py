...
SUPPORT_MATRIX: tuple[dict[str, Any], ...] = (
    {
        ...
        "encryption_scheme": "AES-128-CBC whole-file encrypted SQLite",
        "key_source": "EXTERNAL_KPRAGMA_AND_SERVER_NONCE_OR_RAW_DB_KEY_IV",
        "fixture": "synthetic DFRWS-2019-compatible AES-CBC SQLite fixture",
        "status": "PARTIAL",
        "automatic_key_acquisition": False,
        "source": "DFRWS EU 2019 Choi/Yu/Hyun/Kim case study",
    },
    {
        "platform": "ANDROID",
        "application_version": "*",
...
def _aes_cbc_decrypt(ciphertext: bytes, key: bytes, iv: bytes) -> bytes:
    if len(ciphertext) == 0 or len(ciphertext) % 16 != 0:
        raise ValueError("KakaoTalk ciphertext length is invalid for AES-CBC.")
...
def _extract_chatlogs(plaintext: bytes, *, source_path: str) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="apex-kakaotalk-") as temp_dir:
        ...
        with sqlite3.connect(f"file:{sqlite_path}?mode=ro", uri=True) as connection:
            ...
            if integrity != "ok":
                raise ValueError("KakaoTalk decrypted SQLite integrity check failed.")
            tables = {
                str(row["name"])
            ...
            if "chatLogs" not in tables:
                raise ValueError("KakaoTalk chatLogs table is missing.")
            columns = {
                str(row["name"])
            ...
            if missing_columns:
                raise ValueError("KakaoTalk chatLogs schema is outside the supported version.")
