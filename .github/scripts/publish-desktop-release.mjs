...
function verify() {
  ...
  const ancestry = api(`compare/${config.target_commit}...main`);
  assert.equal(ancestry.merge_base_commit.sha, config.target_commit, 'Target is not merged');
...
if (process.argv[2] === 'verify') {
  ...
  console.log('CI, merged source tree, package version and immutable artifacts verified.');
