...
7. `apex_mcp`: MCP 2.x stdio 및 authenticated Streamable HTTP transport와 M7 Context/View/AI/Report Tool 52개. M8은 Tool surface를 유지하면서 별도 optional dependency, hash lock, console entry point, wheel/sdist, clean-install/Inspector verifier와 Ubuntu/Windows CI를 제공한다. HTTP는 verified bearer principal, frontend actor/session, RBAC/OAuth scope, case allowlist, exact Host/Origin/CORS와 rate limit을 적용하고, 위험 작업은 server-side one-time confirmation이 필수다. Descriptor/transport contract가 없는 Domain service와 legacy schema는 MCP Tool로 노출하지 않는다.

현재 저장소에는 MCP 2.x stdio 서버와 OAuth resource-server 방식의 Streamable HTTP 서버, Context/View binding, 확인 기반 Raw Read 경계, 선택적 OpenAI-compatible AI provider 실행·Core ingest, keyword/scope human verification과 keyword promotion, immutable report draft/review/approval/custody/export manifest 경계가 있다. HTTP human mutation은 모델 제공 actor를 받지 않고 검증된 token principal에 매핑된 frontend actor/session과 case/target/revision/fingerprint가 결합된 일회성 승인을 요구한다. GUI context snapshot 생성은 같은 actor/session에 소유된 live selection만 허용한다. Report export는 승인 ID, immutable content fingerprint와 custody snapshot에 결합되며 renderer result 기록은 host-only다. MCP 패키지의 macOS clean-install·stdio·Inspector는 검증됐고 Windows Python 3.11/3.12 hosted workflow는 구성됐으나, 원격 Windows run과 Desktop bundle 통합 결과는 별도 release evidence가 필요하다. 제품용 identity 발급, 영속 session/approval store, 조직 정책, billing, Desktop GUI와 자율 agent loop는 별도 Backend/Frontend 책임이며 현재 in-memory integration provider를 대체해 같은 port를 구현해야 한다.
