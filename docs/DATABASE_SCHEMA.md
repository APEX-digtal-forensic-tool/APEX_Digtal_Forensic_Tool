...
| `default_timezone` | TEXT | NOT NULL | IANA Timezone |
...
| `job_type` | TEXT | NOT NULL | `HASH`, `FS_INGEST`, `ARTIFACT`, `TIMELINE`, `INDEX`, `SEARCH` |
...
2. Artifact Payload는 생성 후 수정하지 않는다. Parser 변경은 새 Analyzer Run과 새 Artifact를
   만든다.
3. Timeline, Search Document, AI Enrichment, Cache는 재생성 가능한 파생 데이터다.
